 ...
 *         one of 2 I2S interfaces. When PLLI2S is enabled, you need to call HAL_RCCEx_DisablePLLI2S
...
HAL_StatusTypeDef HAL_RCCEx_PeriphCLKConfig(RCC_PeriphCLKInitTypeDef *PeriphClkInit)
{
	...
	if (pllactive == 1) {
		...
		if (HAL_IS_BIT_CLR(RCC->CR, RCC_CR_PLL3ON)) {
			...
			/* Return an error only if user wants to change the PLLI2SMUL whereas PLLI2S
			 * is active */
	...
	#endif /* STM32F102x6 || STM32F102xB || STM32F103x6 || STM32F103xB || STM32F103xE || \
 ...
 * @param  PeriphClkInit pointer to an RCC_PeriphCLKInitTypeDef structure that
 *         returns the configuration information for the Extended Peripherals clocks(RTC, I2S, ADC
 * clocks).
 * @retval None
 */
		...
		if (HAL_IS_BIT_SET(RCC->CR, RCC_CR_PLLON)) {
			...
			defined(STM32F100xE)
				...
				if (HAL_IS_BIT_SET(RCC->CFGR2, RCC_CFGR2_PREDIV1SRC)) {
					...
					/* PLLCLK = PLL2CLK / PREDIV1 * PLLMUL with PLL2CLK =
					 * HSE/PREDIV2 * PLL2MUL */
					...
					/* HSE used as PLL clock source : PLLCLK = HSE/PREDIV1 *
					 * PLLMUL */
