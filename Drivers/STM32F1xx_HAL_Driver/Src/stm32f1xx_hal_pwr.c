 ...
 */
	...
	#define CR_LPSDSR_BB \
		((uint32_t)(PERIPH_BB_BASE + (PWR_CR_OFFSET_BB * 32U) + (LPSDSR_BIT_NUMBER * 4U)))
	...
	#define CSR_EWUP_BB(VAL) \
		((uint32_t)(PERIPH_BB_BASE + (PWR_CSR_OFFSET_BB * 32U) + (POSITION_VAL(VAL) * 4U)))
/**
 * @}
 */
 ...
 * brief   WFE cortex command overloaded for HAL_PWR_EnterSTOPMode usage only (see Workaround
 * section)
 * @{
 */
   ...
   *** Stop mode ***
   =================
    [..]
      ...
      (+) Entry:
	   The Stop mode is entered using the HAL_PWR_EnterSTOPMode(PWR_REGULATOR_VALUE,
PWR_SLEEPENTRY_WFx ) function with:
      ...
      (+) Exit:
	  (++) WFI entry mode, Any EXTI Line (Internal or External) configured in Interrupt mode
with NVIC configured
   ...
   *** Auto-wakeup (AWU) from low-power mode ***
	   ...
	   (++) To wake up from the Stop mode with an RTC alarm event, it is necessary to
		configure the RTC to generate the RTC alarm using the HAL_RTC_SetAlarm_IT()
function.
 ...
 * @param Regulator: Regulator state as no effect in SLEEP mode -  allows to support portability
...
void HAL_PWR_EnterSTOPMode(uint32_t Regulator, uint8_t STOPEntry)
{
	...
	/* Clear PDDS bit in PWR register to specify entering in STOP mode when CPU enter in
	 * Deepsleep */
	CLEAR_BIT(PWR->CR, PWR_CR_PDDS);

	/* Select the voltage regulator mode by setting LPDS bit in PWR register according to
