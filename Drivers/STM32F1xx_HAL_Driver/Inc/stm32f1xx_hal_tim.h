 ...
 */
typedef struct {
	uint32_t OffStateRunMode; /*!< TIM off state in run mode, This parameter can be a value of
				     @ref TIM_OSSR_Off_State_Selection_for_Run_mode_state */
	...
	uint32_t DeadTime; /*!< TIM dead Time, This parameter can be a number between Min_Data =
	...
	uint32_t AutomaticOutput; /*!< TIM Automatic Output Enable state, This parameter can be a
				     value of @ref TIM_AOE_Bit_Set_Reset */
...
#define TIM_ICPOLARITY_BOTHEDGE \
	TIM_INPUTCHANNELPOLARITY_BOTHEDGE /*!< Capture triggered by both rising and falling edges \
					     on timer input*/
/**
 * @}
 */
...
#define TIM_ICSELECTION_DIRECTTI \
	TIM_CCMR1_CC1S_0 /*!< TIM Input 1, 2, 3 or 4 is selected to be connected to IC1, IC2, IC3 \
			    or IC4, respectively */
#define TIM_ICSELECTION_INDIRECTTI \
	TIM_CCMR1_CC1S_1 /*!< TIM Input 1, 2, 3 or 4 is selected to be connected to IC2, IC1, IC4 \
			    or IC3, respectively */
...
#define TIM_ENCODERMODE_TI12 \
	(TIM_SMCR_SMS_1 | \
	 TIM_SMCR_SMS_0) /*!< Quadrature encoder mode 3, x4 mode, counts up/down on both TI1FP1 \
...
#define TIM_COMMUTATION_TRGI \
	TIM_CR2_CCUS /*!< When Capture/compare control bits are preloaded, they are updated by \
			setting the COMG bit or when an rising edge occurs on trigger input */
#define TIM_COMMUTATION_SOFTWARE \
	0x00000000U /*!< When Capture/compare control bits are preloaded, they are updated by \
		       setting the COMG bit */
/**
 * @}
 */
...
#define TIM_CLOCKPRESCALER_DIV2 \
	TIM_ETRPRESCALER_DIV2 /*!< Prescaler for External ETR Clock: Capture performed once every \
				 2 events. */
#define TIM_CLOCKPRESCALER_DIV4 \
	TIM_ETRPRESCALER_DIV4 /*!< Prescaler for External ETR Clock: Capture performed once every \
				 4 events. */
#define TIM_CLOCKPRESCALER_DIV8 \
	TIM_ETRPRESCALER_DIV8 /*!< Prescaler for External ETR Clock: Capture performed once every \
				 8 events. */
/**
 * @}
 */
...
#define TIM_OSSR_ENABLE \
	TIM_BDTR_OSSR /*!< When inactive, OC/OCN outputs are enabled (still controlled by the \
			 timer)           */
#define TIM_OSSR_DISABLE \
	0x00000000U /*!< When inactive, OC/OCN outputs are disabled (not controlled any longer by \
		       the timer) */
/**
 * @}
 */
...
#define TIM_OSSI_ENABLE \
	TIM_BDTR_OSSI /*!< When inactive, OC/OCN outputs are enabled (still controlled by the \
			 timer)           */
...
#define TIM_TRIGGERPRESCALER_DIV2 \
	TIM_ETRPRESCALER_DIV2 /*!< Prescaler for External ETR Trigger: Capture performed once \
				 every 2 events. */
#define TIM_TRIGGERPRESCALER_DIV4 \
	TIM_ETRPRESCALER_DIV4 /*!< Prescaler for External ETR Trigger: Capture performed once \
				 every 4 events. */
#define TIM_TRIGGERPRESCALER_DIV8 \
	TIM_ETRPRESCALER_DIV8 /*!< Prescaler for External ETR Trigger: Capture performed once \
				 every 8 events. */
/**
 * @}
 */
