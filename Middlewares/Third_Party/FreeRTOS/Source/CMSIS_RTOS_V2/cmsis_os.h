...
uint32_t osKernelSysTick(void);
	...
	/// \note Reflects the system timer setting and is typically defined in a configuration
	/// file.
	#if (osCMSIS < 0x20000U)
...
/// \param         instances     number of possible thread instances (used to statically allocate
/// memory). \param         stacksz       stack size (in bytes) requirements for the thread
/// function.
	...
	#define osThreadDef(name, priority, instances, stacksz) \
		...
		const osThreadDef_t os_thread_def_##name = { \
			(name), \
			{NULL, osThreadDetached, (instances == 1) ? (&os_thread_cb_##name) : NULL, \
...
/// pool. \return status code that indicates the execution status of the function.
...
/// \param[in]     thread_id     thread ID (obtained by \ref osThreadCreate or \ref osThreadGetId)
/// or NULL. \return message queue ID for reference by other functions or NULL in case of error.
...
/// \param[in]     thread_id     thread ID (obtained by \ref osThreadCreate or \ref osThreadGetId)
/// or NULL. \return mail queue ID for reference by other functions or NULL in case of error.
